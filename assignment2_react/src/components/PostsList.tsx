    ...
    useEffect(() => {
        const observer = new IntersectionObserver(
            entries => {
                ...
                if (target.isIntersecting && !loading && visiblePosts.length < activePosts.length) {
                    loadMorePosts();
                }
            },
            { 
    ...
    if (!activePosts.length) {
        return (
            <div className="alert alert-info text-center py-4">
                ...
                <p className="mb-0">There are no posts currently published within their active date range.</p>
            </div>
        );
    }
            ...
            <div className="row row-cols-1 g-4 mb-4">
                {visiblePosts.map((post, index) => (
                    <div className="col" key={post.ArticleId}>
                        {/* Featured post styling for the first post */}
                        {index === 0 ? (
                            <div className="card border-0 shadow-sm mb-2 overflow-hidden">
                                ...
                                <div className="row g-0 align-items-center">
                                    <div className="col-md-12">
                                        <div className="card-body d-flex flex-column h-100 p-4">
                                            ...
                                            <p className="card-text">{post.Body.length > 150 ? `${post.Body.substring(0, 150)}...` : post.Body}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
